...
class TextToSpeech_api(tornado.web.RequestHandler):
	def post(self):
		try:
			input = self.get_argument('text', True)
			text = gTTS(input)
...
def make_app():
	return tornado.web.Application([(r"/translate", Translator_api),(r"/tts", TextToSpeech_api)])
